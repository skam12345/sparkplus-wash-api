const insertcouponinfoasgift = (coupon_code, coupon_name, prod_name , coupon_type, dc_fee, dc_percent, end_date, 
     rest_count, plc_code)  =>`
	INSERT INTO coupon_info
		...
		cin_plc_code, wpd_seq_no, cin_publish_type, cin_is_publish, cin_publish_date)
	values
	("${coupon_code}", CONCAT("${coupon_name}",':',"${prod_name}"), "${coupon_type}", "${dc_fee}", 
	"${dc_percent}", 'Y', NOW(), '1900-01-01', 
	"${end_date}", '', "${rest_count}",'', 
    "${plc_code}", 0, 'CPT002', 'N', NOW())
`

module.exports = insertcouponinfoasgift;