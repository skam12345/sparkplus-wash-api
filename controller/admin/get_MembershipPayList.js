...
const selectmembershippaylist = require('../../query/admin/get_membershippaylist/select_membershippaylist');
const updatemembershipdata = require('../../query/admin/get_membershippaylist/update_membershipdata');

module.exports.getmembershippaylist = (req, res) =>{
    let search_date = req.body.search_date;
    var list = '[\n';
    connection.query(updatemembershipdata, (error, rows) =>{
        if (error) throw error;
    })

    connection.query(selectmembershippaylist(search_date), (error, rows) =>{
        if (error) throw error;
        if(rows.length > 0){
